...
namespace sb
{
	void IndexList::remove(Drawable* drawable)
	{
		if (m_drawableInfos.find(drawable) == m_drawableInfos.end() && std::find(m_drawablesToAdd.begin(), m_drawablesToAdd.end(), (drawable)) == m_drawables.end())
	...
	void IndexList::add(Drawable* drawable)
	{
		if (m_drawableInfos.find(drawable) != m_drawableInfos.end() && std::find(m_drawablesToRemove.begin(), m_drawablesToRemove.end(), drawable) == m_drawables.end())
	...
	void IndexList::removeDrawables()
	{
	...
	void IndexList::resizeIndices()
	{
		m_indices.resize(m_indices.size() + m_numIndicesToAdd);
	}

	void IndexList::recalcIndices()
	{
