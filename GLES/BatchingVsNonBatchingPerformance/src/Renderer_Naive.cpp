#include "Renderer.h"
ORB_PLATFORM_SPECIFIC()
#ifdef ORB_USE_NAIVE_RENDERER
...
namespace sb
{
	...
	void Renderer::draw()
	{
		std::size_t counter = 0;
		for (std::size_t i = 0; i < m_drawables.size(); i++) {
			setVertexAttribPointer(m_shader.getAttributeLocation("a_vPosition"), 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), &(m_vertices[counter].position));
			setVertexAttribPointer(m_shader.getAttributeLocation("a_vColor"), 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), &(m_vertices[counter].color));
			counter += m_drawables[i]->mesh.getVertexCount();
			glDrawElements(GL_TRIANGLES, m_drawables[i]->mesh.getIndices().size(), GL_UNSIGNED_SHORT, m_drawables[i]->mesh.getIndices().data());
		}
