...
{
    const int32_t App::POSITION_PARAMETER_INDEX = 0;
    const int32_t App::COLOR_PARAMETER_INDEX = 1;
    
    GLuint App::LoadShader(const char *shaderSrc, GLenum type)
    {
        GLuint shader;
        GLint compiled;
    ...
    int App::init() {
        ...
        glBindAttribLocation(m_shaderProgram, POSITION_PARAMETER_INDEX, "a_vPosition");
        glBindAttribLocation(m_shaderProgram, COLOR_PARAMETER_INDEX, "a_vColor");
    ...
    int App::Run()
    {
        ...
        window = SDL_CreateWindow(NULL, 0, 0, m_width, m_height, SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN | SDL_WINDOW_SHOWN);
